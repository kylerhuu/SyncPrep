...
exports.normalizeDraftToWindows = normalizeDraftToWindows;
 ...
 */
function resolveDraftDate(draftDate, scheduleDays) {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(normalized)) {
        const dt = luxon_1.DateTime.fromISO(normalized, { zone: "utc" });
        if (dt.isValid) {
            const weekday = dt.weekday; // 1 = Monday ... 7 = Sunday
            const byWeekday = scheduleDays.find((d) => {
                const sd = luxon_1.DateTime.fromISO(d.date, { zone: "utc" });
                return sd.isValid && sd.weekday === weekday;
            });
            if (byWeekday)
                return byWeekday.date;
        }
    ...
    if (lower === "today")
        return scheduleDays[0]?.date ?? null;
    if (lower === "tomorrow")
        return scheduleDays[1]?.date ?? null;
    const matchDayName = (label) => {
    ...
    const byShort = scheduleDays.find((d) => d.label.toLowerCase().startsWith(short) || d.label.toLowerCase().includes(lower));
 ...
 */
function normalizeDraftToWindows(draft, scheduleDays) {
    const windows = [];
    const stats = { inRange: 0, skipped: 0, outOfRange: 0 };
